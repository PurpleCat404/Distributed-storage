...
def start_nodes(num_nodes, base_port=5000):
    ...
    for i in range(num_nodes):
        node_key = f"Node{i}"
        port = base_port + i
        p = subprocess.Popen(["python", "node_server.py", "--node-key",
                              node_key, "--port", str(port)])
        processes.append(p)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num-nodes', type=int,
                        default=3, help='Number of nodes to start')
    parser.add_argument('--base-port', type=int,
                        default=5000, help='Base port for starting nodes')
    parser.add_argument('--replication-factor', type=int,
                        default=1, help='Replication factor')
    parser.add_argument('--main-port', type=int,
                        default=8000, help='Port for main server')
    args = parser.parse_args()

    nodes_mapping, processes = start_nodes(args.num_nodes, args.base_port)

    helper = NodesHelper(nodes_mapping=nodes_mapping,
